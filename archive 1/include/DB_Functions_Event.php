...
class DB_Functions_Event {
    ...
    *****************************************************************************/
    ...
    public function addClick($ad, $user) {
        ...
        $stmt1 = $this->conn->prepare("SELECT * FROM diu_ad WHERE ad_id = ? AND user_id = ?");
        ...
        if ($stmt1->num_rows == 0) {
            ...
            $stmt = $this->conn->prepare("INSERT INTO diu_ad(ad_id, user_id) 
                                            VALUES(?, ?)");
            $stmt->bind_param("ii", $event, $user);
            $result = $stmt->execute();
            $stmt->close();
            ...
            // check for successful store
            if ($result) {
                $stmt = $this->conn->prepare("SELECT * FROM diu_ad WHERE ad_id = ? AND user_id = ?");
